...
$queryProduk = mysqli_query($conn, "SELECT id, nama_produk, foto, harga_produk, foto, detail FROM produk limit 5");
?>
...
<body>
	...
	<!-- carousel -->
	<div id="carouselExampleInterval" class="carousel slide" style="margin-top: -20px" data-bs-ride="carousel">
		<div class="carousel-inner" style="height: 565px;">
			<?php
			include '../config.php';

			$queryCarousel = mysqli_query($conn, "SELECT * FROM Slide_Show");
			while ($slide = $queryCarousel->fetch_assoc()) {
			?>
				<div class="carousel-item active" data-bs-interval="3000">
					<?php
					echo	"<img src='../fotoslide/$slide[namaSlide]' class='d-block w-100' alt='banner_legion'>";
					?>
				</div>
			<?php
			}
	...
	<!-- card rekomendasi -->
	<div class="card-main">
		<h2 style="text-align: center; font-family: 'URW'">REKOMENDASI KAMI</h2>
		<br>
		<table>
			<tbody>
				<tr>
					<?php while ($row = $queryProduk->fetch_assoc()) : ?>
						<td>
							<div class="card" style="width: 14rem; margin-right: 15px; margin-left:45px">
								<img src="../fotoproduk/<?= $row['foto']; ?>" class="card-img-top" style="margin-bottom: 1px;" height="150" width="200">
								<div class="card-body">
									<hr>
									...
									<div class="d-grid gap-2 d-md-flex justify-content-md-end">
										<a href="#popup<?= $row['id']; ?>">
											<button class="btn btn-primary btn-sm me-md-2" type="button">Detail</button>
										</a>
										<form method="post">
							...
							<div class="popup" id="popup<?= $row['id']; ?>">
								<div class="popup_content">
									<div class="popup_img">
										<img src="../fotoproduk/<?= $row['foto']; ?>" style="margin-bottom: 1px;" height="320" width="100">
										<a href="#dfa" class="popup_close">&times;</a>
									</div>
									<div class="popup_header">
										<h1><?= $row['nama_produk']; ?></h1>
										<h2>Rp. <?= $row['harga_produk']; ?></h2>
									</div>
									...
									<form method="post">
										...
										<input type="hidden" name="hargaproduk" value="<?= $row['harga_produk']; ?>">
										<input type="hidden" name="foto" value="<?= $row['foto']; ?>">
										<button class="btn btn-light btn-sm" name="submit_add_to_cart" type="submit" value="<?= $row['id']; ?>" style="margin-top: 10px; margin-bottom: 30px; margin-left: 600px;">Add to Cart</button>
									</form>
								</div>
							</div>
						</td>
