...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_add_to_cart'])) {
	...
	header("Location: laptopgaming.php");
	exit;
}

$queryLaptop = mysqli_query($conn, "SELECT P.id, P.nama_produk, P.foto, P.harga_produk, P.foto, P.detail, K.nama FROM produk P JOIN Kategori K ON P.kategori_id=K.id AND K.nama='Laptop Gaming'");
?>
...
<body>
		...
		<table>
			<tbody>
				<tr>
					...
					<td>
						<a href="cart.php" style="text-decoration: none; margin-right: 10px"><i class="fa-solid fa-cart-shopping"></i></a>
					</td>
	...
	<div class="container-fluid py-0 px-0">
		<h2 style="text-align: center; font-family: 'URW'; margin-top: 20px">LAPTOP GAMING</h2>
		<div class="container">
			<div class="row mt-5">
				<?php
				...
				?>
					<div class="col-sm-6 col-md-3 mb-5">
						<div class="card" style="width: 14rem; margin-right: 20px; margin-left:65px">
							<?php
							echo "<img src='../fotoproduk/$row[foto]' class='card-img-top' style='margin-bottom: 1px;' height=150 width=200>";
							?>
							<div class="card-body">
								<hr>
					...
					<div class="popup" id="popup<?php echo $row['id']; ?>">
						<div class="popup_content">
							<div class="popup_img">
								<<?php
									echo "<img src='../fotoproduk/$row[foto]' class='card-img-top' style='margin-bottom: 1px;' height=320 width=100>";
									?>
							...
							<form action="" method="post">
								...
								<button class="btn btn-light btn-sm" name='submit' type="submit" style="margin-top: 10px; margin-bottom: 30px; margin-left: 600px;" value="<?php echo $row['id']; ?>">Add to Cart</button>
							</form>
						</div>
						<?php
						include '../config.php';
						if (isset($_POST['submit'])) {
							...
							$hargacard = $_POST['hargaproduk'];
							$stokcard = 1;
							$totalcard = $_POST['hargaproduk'];

							$queryCheck = mysqli_query($conn, "SELECT quantity, total_harga FROM Keranjang WHERE username='$usercard' AND id='$idcard'");
							...
							if ($countCheck > 0) {
								...
								$queryUpdate = mysqli_query($conn, "UPDATE Keranjang SET quantity='$newQuantity', total_harga='$newTotal' WHERE username='$usercard' AND id='$idcard'");
							} else {
								$queryCart = mysqli_query($conn, "INSERT INTO keranjang VALUES ('$usercard', '$idcard', '$fotocard', '$namacard', '$hargacard', '$stokcard', '$totalcard')");
							}

						?>
							<meta http-equiv="refresh" content="0; url=laptopgaming.php" />
						<?php
						}
						?>
					</div>
