    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <!-- End of Topbar -->
                <div class="mt-2">
                    <h3 style="text-align:center">Tambah Data Pelanggan</h3>
                    ...
                    if(isset($_POST["submit"])){
                        $user = $_POST['username'];
                        $pass = $_POST['password'];
                        $sqluser = mysqli_query($conn, "SELECT username FROM Pelanggan WHERE username='$user'");
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <?php 
                                    ...
                                    $sql    = "SELECT * FROM Pelanggan WHERE username NOT IN (SELECT username FROM Keranjang) ";
                                    $result = $conn->query($sql);

                                    while ($row = $result->fetch_assoc()) 
                                    {
                                        echo '<tr>';
                                        echo '<br><td>'.$row['idPelanggan'].'</td>';
                                        echo '<td>'.$row['username'].'</td>';
                                        echo '<td>'.$row['password'].'</td>';
                                        echo '<td>'; ?> 
                                        
                                        <a href="" class="btn btn-sm btn-info" data-toggle="modal"
                                            data-target="#modal<?php echo $row['idPelanggan']; ?>">Edit</a>
                                        <div class="modal fade" id="modal<?php echo $row['idPelanggan']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel"
                                            aria-hidden="true">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <form method="POST">
                                                            <div class="form-group">
                                                                ...
                                                                <input type="text" name="id" class="form-control" value="<?php echo $row['idPelanggan']; ?>" disabled>
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput1">Username Pelanggan</label>
                                                                <input type="text" name="user" class="form-control" value="<?php echo $row['username']; ?>">
                                                            </div>
                                                            <div class="form-group">
                                                                <label for="exampleFormControlInput1">Password Pelanggan</label>
                                                                <input type="text" name="pass" class="form-control" value="<?php echo $row['password']; ?>">
                                                            </div>
                                                            <button type="submit" name="buttonedit" class="btn btn-primary" value="<?php echo $row['idPelanggan']; ?>">Save changes</button>
                                                            <?php 
                                                                    ...
                                                                    $update = mysqli_query($conn, "UPDATE Pelanggan SET username='$useredit', password='$passedit' WHERE idPelanggan='$idedit'");
                                                                    ?>
                                        ...
                                        <form action="" method="POST">
                                            <button class="btn btn-danger" type="submit" name="buttonhapus" value="<?php echo $row['username']; ?>">Hapus</button>
                                            <?php 
                                                if(isset($_POST['buttonhapus'])){
                                                    $id = $_POST['buttonhapus'];
                                                    
                                                    $queryhapus = mysqli_query($conn, "DELETE FROM Pelanggan WHERE username='$id'");
                                                    $queryhapusKeranjang = mysqli_query($conn, "DELETE FROM Keranjang WHERE username='$id'");
                                                    ?>
                                                    <meta http-equiv="refresh" content="0; url=pelanggan.php"/>
                                                    <?php
                                                }
